...
export default class LoginForm extends PanElement {
  ...
  readonly #session: { value: string, valid: boolean } = { value: '', valid: true }

  render(): TemplateResult {
    const updateUsername = ({ target: { value } }: { target: HTMLInputElement }): unknown => this.username = value
    const updateSession = ({ target: { value } }: { target: HTMLInputElement }): unknown => this.session = value
    ...
    return html`<div class="bg-base-200 p-8 rounded shadow-md w-full sm:w-96">
      ...
      <form class="space-y-5">
        <label class="input flex items-center gap-2 ${this.#username.dirty && (this.#username.valid ? 'input-success' : 'input-error')}">
          ...
          <input type="text" id="username" name="username" .value=${this.#username.value} class="grow" placeholder="Username" required @input=${updateUsername} />
        </label>
        <div class="indicator w-full">
          ...
          <label class="input flex items-center gap-2 w-full ${this.#session.value !== '' && (this.#session.valid ? 'input-success' : 'input-error')}">
            ...
            <input type="text" id="session" name="session" .value=${this.#session.value} class="grow" placeholder="Session ID" @input=${updateSession} />
          </label>
        </div>
