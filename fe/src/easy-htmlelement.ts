...
export default class EasyHTMLElement {
  ...
  public readonly elem!: HTMLElement | SVGElement
  ...
  public static anchor({ href, text }: { href: string, text: string }): EasyHTMLElement {
    return new EasyHTMLElement('a').attrs({ href }).content(text)
  }
  ...
  public on<K extends keyof HTMLElementEventMap>(event: K, listener: (this: HTMLElement, ev: HTMLElementEventMap[K]) => unknown): this {
    this.elem.addEventListener(event, listener as EventListener)
    ...
    this.elem.append(...contents
      ...
      .flatMap(content => (typeof content !== 'string' ? content.elem : content
        ...
        .flatMap(fragment => (typeof fragment === 'string'
          ? fragment.split(/\n/g).flatMap(t => [new EasyHTMLElement('br').elem, t]).slice(1)
